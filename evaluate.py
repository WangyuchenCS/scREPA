...
import random
import torch
sc.settings.figdir = "/home/grads/ywang2542/Perturbation/Embedding/scDistillOTC/scDistillOTC/figures"
...
def evaluate_adata(eval_adata, cell_type, key_dic, random_seed = 42):
    ...
    :param key_dic: dictionary of keywords for the dataset, such as
               ...
               'ctrl_key': 'control',
               'stim_key': 'stimulated', #perturbed
               'pred_key': 'predict',
               }
    :return: None
    """
    # PCA cluster results
    sc.tl.pca(eval_adata, random_state=42)  # 8001 × 6998  random_seed
    sc.pl.pca(eval_adata, 
              color=key_dic['condition_key'], 
              frameon=False,
              title="PCA of " + cell_type + " by Conditions", 
              return_fig=False, 
              show=True, 
              save="_" + cell_type + " by Conditions.pdf",
              )
    # # UMAP cluster results
    # sc.pp.neighbors(eval_adata, use_rep="X_pca")  # 基于 PCA 空间构建邻接图
    # sc.tl.umap(eval_adata, random_state=42)
    # fig_umap = sc.pl.umap(
    #     eval_adata,
    #     color=key_dic['condition_key'],
    #     frameon=False,
    #     show=True,
    #     title="UMAP of " + cell_type + " by Conditions",
    #     return_fig=False,
    #     save="_" + cell_type + "_by_Conditions.pdf"
    # )

    # conmmon degs of top 100
    sc.tl.rank_genes_groups(eval_adata, groupby=key_dic['condition_key'],
                            reference=key_dic['ctrl_key'], method="wilcoxon", random_state=42)  # random_seed
    ...
    print(cell_type, " common DEGs: ", common_nums)  #'CD4T':60
    ...
    # violin contrast
    fig_dir = "/home/grads/ywang2542/Perturbation/Embedding/scDistillOTC/scDistillOTC/figures"
    gene = degs_ctrl[0]
    sc.pl.violin(eval_adata, keys=gene, groupby=key_dic['condition_key'], show=True, save=f"_{cell_type}_{gene}.pdf")
    # plt.savefig(f"{fig_dir}/violin_{cell_type}_{gene}.pdf", dpi=300)
    # plt.close()
    gene = degs_ctrl[1]
    sc.pl.violin(eval_adata, keys=gene, groupby=key_dic['condition_key'], show=True, save=f"_{cell_type}_{gene}.pdf")
    # plt.savefig(f"{fig_dir}/violin_{cell_type}_{gene}.pdf", dpi=300)
    ...
    sc.tl.rank_genes_groups(eval_adata, groupby=key_dic['condition_key'],
                            reference=key_dic['ctrl_key'], method="wilcoxon", random_state=42)  # random_seed
    sc.pl.rank_genes_groups(eval_adata, n_genes=25, sharey=False, show=True, save=f"_{cell_type}_degscontrast.pdf")

    marker_genes = degs_ctrl[0:20]
    sc.pl.dotplot(eval_adata, marker_genes, groupby=key_dic['condition_key'], show=True, save=f"_{cell_type}_{gene}.pdf")
    # plt.savefig(f"{fig_dir}/dotplot_{cell_type}_{gene}.pdf", dpi=300)
    # plt.close()
    # return degs_pred
...
def get_pearson2(eval_adata, key_dic, n_degs=100, sample_ratio=0.8, times=100, random_seed = 42):
    random_seed = random_seed
    """
    ...
    """
    stim_key = key_dic['stim_key']  # 'stimulated'
    pred_key = key_dic['pred_key']  # 'predict'
    ctrl_key = key_dic['ctrl_key']  # 'control'
    condition_key = key_dic['condition_key']
    sc.tl.rank_genes_groups(eval_adata, groupby=condition_key, reference=ctrl_key, method="wilcoxon", random_state=42)  #random_seed
    ...
    for i in range(times):
        stim = df_stim.sample(frac=sample_ratio, random_state=i)  # 492
        pred = df_pred.sample(frac=sample_ratio, random_state=i)  # 1557
...
def draw_reg_plot(eval_adata,
                ...
                #   axis_keys={"x": "predict", "y": "stimulated"},
                  axis_keys={"x": "predict", "y": "perturbed"},
                  condition_key='condition',
                  gene_draw=None,
                  top_gene_list=None,
                  save_path=None,
    ...
    :param axis_keys: {"x": "predict", "y": "stimulated"}, maybe you need change the values of keys
    ...
    if title:
        ...
        plt.title('Expression ' + reg_type + ' of ' + cell_type)
    if save_path is None:
        fig_dir = "/home/grads/ywang2542/Perturbation/Embedding/scDistillOTC/scDistillOTC/figures"
        save_path = f'{fig_dir}/Expression ' + reg_type + ' of ' + cell_type+'.pdf'
...
def evaluate(data_name, eval_adata, key_dic, save_path=None, return_fig=False, eval_path=None, random_seed =0):
    random_seed = random_seed
    '''
    ...
    stim_key = key_dic['stim_key']  # 'stimulated'
    pred_key = key_dic['pred_key']  # 'predict'
    cell_type_key = key_dic['cell_type_key']  # 'cell_type'
    condition_key = key_dic['condition_key']  # 'condition' 
    types = list(set(adata.obs[cell_type_key]))  # ['CD4T']
    types.sort()
    degs_list = []
    r2_mean_list = []
    r2_std_list = []
    ...
    for cell_type in types:
        ...
        r2_std_list.append(r2_std.values.tolist())
         # 1.PCA
        sc.tl.pca(eval_adata, random_state=42) # random_seed
        fig = sc.pl.pca(eval_adata, 
                        color=condition_key, 
                        frameon=False, 
                        show=True,
                        title="PCA of " + cell_type + " by Conditions", 
        ...
        # # 2.UMAP
        # # sc.tl.pca(eval_adata, random_state=42)
        # sc.pp.neighbors(eval_adata, use_rep="X_pca")  # 基于 PCA 空间构建邻接图
        # sc.tl.umap(eval_adata, random_state=42)
        # fig_umap = sc.pl.umap(
        #     eval_adata,
        ...
        for i in range(3):
            fig = plt.figure(figsize=(6, 3)) # plt.figure()
            ax = fig.add_axes([0, 0, 1, 1])
            gene = degs_ctrl[i]

            sc.pl.violin(eval_adata, keys=gene, groupby=condition_key, ax=ax, show=True, save=f"_{cell_type}_{gene}.pdf") 
    ...
    if return_fig and save_path:  
        with PdfPages(save_path + '/images/{}.pdf'.format(cell_type)) as pdf:
            for i in range(len(fig_list)):
                pdf.savefig(figure=fig_list[i], dpi=300, bbox_inches='tight')
                plt.close()
    return df
